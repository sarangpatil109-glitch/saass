...
export function SalesSettingsForm({ profile }: { profile: any }) {
        ...
        <Card className="p-6 bg-white dark:bg-gray-900 shadow-sm border border-gray-100 dark:border-gray-800">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Input name="phone" type="tel" defaultValue={profile.phone || ''} placeholder="e.g. 9876543210" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">WhatsApp Number</label>
              <Input name="whatsapp_number" type="tel" defaultValue={profile.whatsapp_number || ''} placeholder="e.g. 9876543210" />
            </div>
          </div>
        </Card>
        ...
        <Card className="p-6 bg-white dark:bg-gray-900 shadow-sm border border-gray-100 dark:border-gray-800">
          ...
          <div>
             {profile.profile_photo && (
               <img src={profile.profile_photo} alt="Photo" className="w-24 h-24 object-cover rounded-full bg-gray-50 border mb-3" />
             )}
             <Input type="file" name="profile_photo" accept="image/*" />
             <p className="text-xs text-gray-500 mt-1">Upload a new photo to replace the existing one.</p>
          </div>
        </Card>
        ...
        <Card className="p-6 bg-white dark:bg-gray-900 shadow-sm border border-gray-100 dark:border-gray-800">
          ...
          <div className="space-y-3">
             <label className="flex items-center space-x-3">
               ...
               <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Email notifications for new leads</span>
             </label>
             <label className="flex items-center space-x-3">
               <input type="checkbox" defaultChecked className="rounded border-gray-300 text-blue-600 focus:ring-blue-500" />
               <span className="text-sm font-medium text-gray-700 dark:text-gray-300">WhatsApp notifications for follow-ups</span>
             </label>
             <label className="flex items-center space-x-3">
               <input type="checkbox" defaultChecked className="rounded border-gray-300 text-blue-600 focus:ring-blue-500" />
               <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Commission updates</span>
             </label>
          </div>
        </Card>
