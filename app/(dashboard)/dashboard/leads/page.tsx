...
export default async function (props: { searchParams: Promise<any> }) {
  ...
  let leadsQuery = applyDateFilter(supabase.from('leads').select(`
    *,
  ...
  if (profile?.role === 'vendor') {
    ...
    const { data: execRec } = await supabase.from('sales_executives').select('id').eq('id', (user?.id || '')).single()
    if (execRec) {
  ...
  const { data: salesExecs } = await applyDateFilter(supabase.from('sales_executives').select('id, full_name'), searchParams).eq('status', 'Active')
