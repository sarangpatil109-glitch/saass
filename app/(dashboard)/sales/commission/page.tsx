...
export default async function SalesExecWalletPage() {
  ...
  const { data: exec } = await supabase.from('sales_executives').select('id, full_name, status').eq('user_id', (user?.id || '')).single()
  ...
  // Fetch Commission
  const { data: commissions } = await supabase.from('commission_transactions').select('*').eq('sales_executive_id', exec.id).eq('recipient_type', 'sales_executive').order('created_at', { ascending: false })
  
  const allCommissions = commissions || []
  
  const totalPaid = allCommissions.filter(c => c.status === 'Paid').reduce((acc, curr) => acc + Number(curr.amount), 0)
  const pendingBalance = allCommissions.filter(c => c.status === 'Pending').reduce((acc, curr) => acc + Number(curr.amount), 0)
              ...
              {allCommissions.length > 0 ? (
                allCommissions.map((l: any) => (
                  <tr key={l.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                    ...
                    <td className="px-6 py-4 font-mono text-xs">{l.id.slice(0, 8)}</td>
                    <td className="px-6 py-4 font-medium">
                      <span className={l.status === 'Paid' ? 'text-green-600' : 'text-orange-600'}>
                        {l.status}
                      </span>
                    </td>
