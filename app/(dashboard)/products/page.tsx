...
export default async function ProductsPage({ searchParams }: { searchParams: any }) {
  ...
  const { data: catData } = await supabase.from('products').select('category').is('deleted_at', null)
  const distinctCategories = Array.from(new Set(catData?.map(c => c.category).filter(Boolean)))

  // Build paginated query
  let productsQuery = supabase.from('products').select('*', { count: 'exact' }).is('deleted_at', null)
  ...
  else if (sort === 'price_asc') productsQuery = productsQuery.order('price_monthly', { ascending: true })
  else if (sort === 'price_desc') productsQuery = productsQuery.order('price_monthly', { ascending: false })
  ...
  const { data: allProducts } = await supabase.from('products').select('status').is('deleted_at', null)
  const totalOverall = allProducts?.length || 0
  const publishedProducts = allProducts?.filter((p: any) => p.status === 'Published').length || 0
  const draftProducts = allProducts?.filter((p: any) => p.status === 'Draft').length || 0
  const archivedProducts = allProducts?.filter((p: any) => p.status === 'Archived').length || 0
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card className="p-4 bg-white dark:bg-gray-900 shadow-sm border border-gray-100 dark:border-gray-800">
          <div className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Published</div>
          <div className="text-2xl font-bold text-green-600 dark:text-green-400">{publishedProducts}</div>
        </Card>
        <Card className="p-4 bg-white dark:bg-gray-900 shadow-sm border border-gray-100 dark:border-gray-800">
          <div className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Drafts</div>
          <div className="text-2xl font-bold text-gray-900 dark:text-white">{draftProducts}</div>
        </Card>
        <Card className="p-4 bg-white dark:bg-gray-900 shadow-sm border border-gray-100 dark:border-gray-800">
          <div className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Archived</div>
          <div className="text-2xl font-bold text-orange-600 dark:text-orange-400">{archivedProducts}</div>
        </Card>
      </div>
              ...
              {products && products.length > 0 ? (
                products.map((product: any) => (
                  <tr key={product.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-gray-500 overflow-hidden">
                          {product.logo_url ? <img src={product.logo_url} alt="" className="w-full h-full object-cover rounded" /> : <Tag className="h-5 w-5" />}
                        </div>
                        <div>
                          <div className="font-medium text-gray-900 dark:text-white">{product.name}</div>
                          <div className="text-xs text-gray-500 dark:text-gray-400 font-mono mt-0.5">{product.slug}</div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="font-medium text-gray-900 dark:text-white">₹{product.price_monthly?.toLocaleString()} /mo</div>
                      <div className="text-xs text-gray-500 dark:text-gray-400">₹{product.price_yearly?.toLocaleString()} /yr</div>
                    </td>
                    <td className="px-6 py-4">
        ...
        {totalPages > 1 && (
          <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Link href={`/dashboard/products?page=${Math.max(1, page - 1)}&q=${query}&status=${statusFilter}&category=${categoryFilter}&sort=${sort}`}
              ...
              <Link href={`/dashboard/products?page=${Math.min(totalPages, page + 1)}&q=${query}&status=${statusFilter}&category=${categoryFilter}&sort=${sort}`}
