...
export default async function VendorWalletPage(props: { searchParams: Promise<any> }) {
  ...
  const lifetimeEarnings = allCommissions.reduce((acc, curr) => {
  ...
  const totalPaid = allPayments.reduce((acc, curr) => acc + Number(curr.amount || 0), 0)
                ...
                allCommissions.map((c: any) => {
                  ...
                  <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                    ...
                    <td className="px-6 py-4 font-bold text-gray-900 dark:text-white">
                      ₹{Number(c.vendor_amount).toLocaleString()}
                    </td>
