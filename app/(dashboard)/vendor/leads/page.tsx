...
export default async function (props: { searchParams: Promise<any> }) {
  ...
  `), searchParams).is('deleted_at', null).eq('assigned_vendor_id', vendor.id).order('created_at', { ascending: false })

  const { data: leads } = await leadsQuery

  // Fetch active sales executives for this vendor for filtering
  const { data: salesExecs } = await applyDateFilter(supabase.from('sales_executives')
    .select('id, full_name'), searchParams)
