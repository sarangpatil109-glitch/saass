...
-- ==========================================
-- SAASS Vendor Module - Complete Fix
-- Run this in your Supabase SQL Editor
-- ==========================================

-- 1. Drop existing tables if they exist to start fresh
DROP TABLE IF EXISTS vendor_activity_logs CASCADE;
DROP TABLE IF EXISTS vendor_coupon_codes CASCADE;
...
-- 2. Create the main Vendors table
...
-- 3. Create Vendor Users table for Authentication linking
-- This solves the issue of linking auth.users to vendors securely
...
-- 5. Recreate Vendor Products Junction
...
-- 7. Create Policies
-- Admin Access (Assuming Admins bypass RLS or use Service Role, but we add basic policies)
CREATE POLICY "Enable all access for admin users on vendors" ON vendors FOR ALL TO authenticated USING (true);
CREATE POLICY "Enable all access for admin users on vendor_users" ON vendor_users FOR ALL TO authenticated USING (true);
CREATE POLICY "Enable all access for admin users on vendor_logs" ON vendor_activity_logs FOR ALL TO authenticated USING (true);
CREATE POLICY "Enable all access for admin users on vendor_products" ON vendor_products FOR ALL TO authenticated USING (true);
